...
public class ArmorListener implements Listener
{
    ...
    // FIXME: essentials is still cancelling this event
    ...
    public void playerInteractEvent(PlayerInteractEvent e)
    {
        if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK)
        {
            ...
            // skulls don't equip when right clicked from hotbar
            // FIXME: this check could be somewhere else
            // FIXME: line 62 causes errors
            if (e.getItem() != null)
    ...
    public final void onInventoryClick(final InventoryClickEvent e)
    {
        ...
        {
            return;// TODO: Why does this get called if nothing happens??
        }
        ...
        if (shift)
        {
            ...
            if (newArmorType != null)
            {
                ...
                // TODO: look into simplifying these
                if (newArmorType.equals(ArmorType.HELMET) &&
                        (equipping ? isAirOrNull(e.getWhoClicked().getInventory().getHelmet()) : !isAirOrNull(e.getWhoClicked().getInventory().getHelmet())) ||
                        newArmorType.equals(ArmorType.CHESTPLATE) &&
                                (equipping ? isAirOrNull(e.getWhoClicked().getInventory().getChestplate()) : !isAirOrNull(e.getWhoClicked().getInventory().getChestplate())) || newArmorType.equals(ArmorType.LEGGINGS) &&
                        (equipping ? isAirOrNull(e.getWhoClicked().getInventory().getLeggings()) : !isAirOrNull(e.getWhoClicked().getInventory().getLeggings())) ||
                        newArmorType.equals(ArmorType.BOOTS) &&
        ...
        else
        {
            ...
            if (newArmorType != null && e.getRawSlot() == newArmorType.getSlot())
            {
                ...
                // if the armor equip event is cancelled, it means that the armour will go back to where it was.
                // so we need to cancel this event too, since it changed where the armor was.
                // TODO: this might be a race condition, and could case bugs later. Investigate this.
                if (armorEquipEvent.isCancelled())
                    e.setCancelled(true);
            }
        }
    ...
    private Location shift(Location start, BlockFace direction, int multiplier)
    {
        ...
        return new Location(start.getWorld(),
                start.getX() + direction.getModX() * multiplier,
                start.getY() + direction.getModY() * multiplier,
                start.getZ() + direction.getModZ() * multiplier);
    }
